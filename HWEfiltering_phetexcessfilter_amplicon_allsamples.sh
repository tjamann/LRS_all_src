...
KEEP=keep_seq.txt
GATK=insertion_ampliseq_filtered_sorted_taxt_9May.recode.vcf
GATKGZ=insertion_ampliseq_filtered_sorted_taxt_9May.recode.vcf.gz
PVAL=0.01
COFF=0.15
...
echo "GATK filtering"
python filterphetamplicon.py -vcf_list insertion_ampliseq_filtered_sorted_taxt_9May.recode.vcf -pval_file GATK.hwe -amp_file ampliconregions.csv -cutoff $COFF --pval $PVAL
...
vcftools --gzvcf PHETE-FILTERED_$GATKGZ --remove-filtered-all --recode --recode-INFO-all --out HWfilteredampliconinsertion
...
bgzip -d PHETE-FILTERED_insertion_ampliseq_filtered_sorted_taxt_9May.recode.vcf.gz
